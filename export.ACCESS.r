...
# PER A MILLORAR: DE MOMENT NO ARREGLA LES VARIABLES EN FORMAT DATA AMB HORES:MINUTS:SEGONS (EN ACCESS QUEDA COM UN REAL PROCEDENT DEL FORMAT DE R).

source(file.path(RutinesLocals,"arregla.formats.r"))
source(file.path(RutinesLocals,"carrega.llibreria.r"))
carrega.llibreria("RODBC")
  

export.ACCESS<-function(taula,file.mdb,table.name,table.dict=NULL,fix.formats=TRUE){

# taula: data.frame amb les dades per a exportar a ACCESS
# file.mbd: character, especifica el path i el filename de la base de dades EXISTENT! de ACCESS on va a parar 'taula'.
# table.name: nom de la taula en el ACCESS
# table.dict: NULL - no crea cap diccionari, !NULL - crea una taula que sera el diccionari de 'taula' amb el nom especificat com a caracter.
