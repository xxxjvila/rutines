...
carrega.llibreria(c("foreign","Hmisc"))

if (!"read.spss4"%in%ls()) source(file.path(RutinesLocals,"read.spss4.r"))  
...
control.qualitat<-function(file=NULL,data.intern=FALSE,max.valors=10,desc=NULL,max.rare.values=4,max.factor=max.valors,digits=1,file.res="xxx.xls"){
  ...
  # funció de retorna en format dd/mm/yyyy les dates (anys en quatre digits)
  format.data<-function(x) paste(as.character(days(x)),"-",as.character(months(x)),"-",as.character(years(x)),sep="")

  

  # funció que retorna valors rars segons hclust.
  ...
  # funció que retorna les etiquetes i els valors
  labels<-function(x){
    value.labels<-value.labels[[i]]
    t<-table(x)
  ...
  if (is.null(file)){  ## selecció del fitxer de forma interactiva.
    inter=TRUE
  ...
  ##### BUCLE PER A TOTES LES VARIABLES NO DESCARTADES (DESC) #####
	...
	for (i in mirar){
		...
		## IDENTIFICACIÓ DE LA CATEGORIA DE LA VARIABLE 
		# categoria = 1: numèrica
		# categoria = 2: cadena
		# categoria = 3: data.
    ...
    if (n.missing<n){
      ...
      if (categoria == 1){
        ...
        if (length(t)>max.valors){
          ...
          if (!is.null(attr(var,"value.labels"))){
            ...
            valors.eliminats<-c(valors.eliminats,as.integer(as.character(lab.values[,1]))) # servirà per a fer descriptius (eliminarem els valors etiquetats).
            # si gairebé tots els valors estan etiquetats, encara que n'hi hagin més de max.valors fa una taula de freqüències.
        ...
        if (length(t)<=max.valors || fer.taula.freq){
          ...
          ## TAULA DE FREQÜÈNCIES AMB VALORS I VALUE.LABELS 
          aux<-taula[[i]]$taula.freq<-labels(var)
          if(sum(aux[,1]%in%aux[,2])==nrow(aux)) taula[[i]]$taula.freq=taula[[i]]$taula.freq[,-2]  # si passa això és que no hi havia etiquetes (eliminem la columna de labels) 
      ...
      if (categoria == 2){  # o fa una taula de freqüencies o no fa res.
        if (length(t)<=max.factor){
          aux<-taula[[i]]$taula.freq<-data.frame(list(values=names(t),freq=as.integer(t)))
          perc<-100*aux[,2]/(n-n.missing)
          perc<-format(round(perc,digits),trim=TRUE,nsmall=digits)
          taula[[i]]$taula.freq$perc=perc
        }else taula[[i]]$taula.freq<-NULL  # no dóna la taula de freqüències per haver-ho massa valors diferents (max.factor).
      }
      ...
      if (categoria == 3){
        ...
        } else{   # fa una taula de freqüències
