##### DIBUIXA UNA CURVA ROC I DONA L'AUC I EL SEU INTERVAL DE CONFIAN?A #######


source(file.path(RutinesLocals,"carrega.llibreria.r"))
carrega.llibreria(c("Hmisc","ROCR"))
...
print.ROC<-function(obj){
  cat("--- Area ander ROC curve [95%CI] ---- \n")
  AUC<-obj["AUC"]
  #print.default(obj)
  AUC.inf<-obj["inf"]
  AUC.sup<-obj["sup"]
  cat(paste("AUC = ",round(AUC,3)," [",round(AUC.inf,3)," ; ",round(AUC.sup,3),"]",sep=""),"\n\n")
}

  
plot.ROC<-function(obj,identify=FALSE,plot.text=FALSE,...){
  p<-attr(obj,"p")
  y<-attr(obj,"y")
  AUC<-obj["AUC"]
  AUC.inf<-obj["inf"]
  ...
  res<-paste("AUC = ",round(AUC,3)," [",round(AUC.inf,3)," ; ",round(AUC.sup,3),"]",sep="")
	...
	#windows()
	par(las=1)
	plot(perf,type="l",ylim=c(0,1),xlim=c(0,1),ylab="Sensitivity",xlab="1-Specificity",main="",xaxs="i",yaxs="i",...)
	abline(0,1,lty=2)
  ...
  if (identify){
    while (winDialog("yesno","Vols identificar riscos en la curva ROC?")=="YES"){
  		...
  		winDialog("ok",paste("El risc associat es: ",round(100*p[index],2),"%",sep=""))
  	}
  	plot(1-esp,sens,type="l",ylim=c(0,1),xlim=c(0,1),ylab="Sensitivity",xlab="1-Specificity",main="ROC curve",xaxs="i",yaxs="i")
  	abline(0,1,lty=2)
  	text(0.6,0.25,res)
  }
  return(res)
