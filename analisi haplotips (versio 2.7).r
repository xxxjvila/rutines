...
bar.plot<-function(type,title=grup.label,cex.legend=1,cex.table=1,cex.screen=0,dens=20,
	...
	if (!is.null(xtext) & !is.null(coord.xtext)){	
		par(xpd=NA)
...
while (tornar){
	...
	if (canvi.resposta){
		...
		y.label<-select.list(c("None",nom.variables),"None",title="Variable resposta")
	...
	if (canvi.grup){
		grup.label<-select.list(c("None",nom.variables),"None",title="Variable grup")
	...
	if (canvi.min.freq){
		min.freq<-winDialogString(message="Introdueix freq. mínima dels haplotips (en tant per 1: ex. 0.01 si es vol el 1%)", default="0.01")
				...
				if (tipus>0){
					...
					while (!plot.acabat){
						...
						if (sel=="Posició llegenda"){
							...
							legend.coord=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						...
						if (sel=="Títol eix x"){
							...
							coord.xtext=getGraphicsEvent("Posiciona el títol",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Separació de les barres"){
							...
							bar.space=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Separació files llegenda"){
							...
							legend.intersp=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Separació files taula"){
							...
							table.intersp=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Mida llegenda"){
							...
							cex.legend=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Llargada taula"){ 
							...
							cex.screen=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Mida taula"){
							mousemove <- function(buttons, x, y){
							...
							cex.table=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Densitat"){
							mousemove <- function(buttons, x, y){
							...
							dens=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Mida dels valors"){
							...
							cex.values=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Alçada títol taula"){
							...
							table.title.w=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
						if (sel=="Mida títol taula"){
							...
							table.title.cex=getGraphicsEvent("",onMouseMove=mousemove,onMouseDown=mousedown)
						}
