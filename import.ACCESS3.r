...
import.ACCESS3<-function(file.mbd,nom.taula,nom.variables=list("ALL"),nom.dicc="",file.spss="",var.dicc="",noms.taules=gsub(" ","_",nom.taula),fix.formats=TRUE){
  ...
  for (i in 1:length(nom.taula)){
    ...
    if (nom.dicc!=""){ # va etiquetant les taules
      ...
      if (length(var.dicc)==4) dict2<-subset(dict,tolower(dict[,var.dicc[4]])==tolower(nom.taula.access))  # els noms de les taules no son case-sensitive.
      ...
      for (j in which(names(taula)%in%comuns)){
        ...
        if (!is.na(value.labels) & value.labels!=""){
          ...
          value.labels<-gsub(" *; *",";",value.labels)
          value.labels<-unlist(strsplit(value.labels,";"))
          value.labels<-strsplit(value.labels,"=")
          value.labels<-lapply(value.labels,function(x){
            out<-c(paste(x[-length(x)],collapse="="),x[length(x)])
            paste(" \" ",out[1]," \" = ",out[2])
            })
          value.labels<-paste(unlist(value.labels),collapse=",")
    ...
    if (fix.formats) taula<-arregla.formats(taula)
...
## Exemple:
#import.ACCESS3(
#file.mbd="U:\\ULEC\\Software\\R cran project\\rutines\\bd1.mdb",
#nom.taula=c("dades"),
#nom.variables=list(c("ALL")),
#nom.dicc="diccionari",
#file.spss="",
#var.dicc=c("nom","etiqueta","etiq_val","taula"),
#noms.taules=c("dades"),
#fix.formats=TRUE)
