########### IMPORTA UNA TAULA DE ACCESS (SENSE ARREGLAR FORMATS) ################
...
carrega.llibreria(c("RODBC","chron"))



#### importa.ACCESS: importa diferents taules d'una base de dades de ACCESS .mbd ####
...
import.ACCESS<-function(file,nom.taula,nom.variables=list("*"),nom.taula.R=nom.taula){  # per defecte ho fa interactivament
  ...
  #     però si no són comptatibles amb R caldrà modificarlos (espais en blanc, ')', '(', etc... )

  
  # arregla els noms de les taules perquè siguin compatibles amb R
  ...
  for (i in 1:length(nom.taula)){
    ...
    if (!is.character(taula)){ ## @@@ arreglar quan la consulta a ACCESS doni error (taula serà un character).......
      ...
      # afegim els atributs no estàndards (els estandards són class, names i rownames)
