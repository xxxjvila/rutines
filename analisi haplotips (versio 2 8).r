...
path.llibreries<-choose.dir("Selecciona la carpeta on tens les llibreries locals (CANCEL si tens connexió a internet)")
...
while (tornar){
	...
	if (canvi.filtre){  # keep.filtre
		...
		if (!filtre=="None"){
			...
			filtre<-sub("=","==",filtre)   ##@
			write(paste("keep.filtre<-",filtre,sep=""),fff<-file.path(tempdir(),"xxx.r"))
			#file.show("xxx.r")
			source(fff)
		}
