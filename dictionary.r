...
if (!"spss_varlist"%in%ls()) source(file.path(RutinesLocals,"spss_varlist.r")) 
...
# files: bases de dades SPSS (.sav) de la qual extreiem la informació de les vari i value labels.
# file.res: arxiu de sintaxis (.sps) on va a parar tota aquesta informació.
  ...
  if (!is.null(files) && length(files)>0){ 
    ...
    if (inter){
      ...
      file.res<-winDialogString(message="Arxiu de resultats (recorda posar l'extensió .sps)", default=file.res.default)
    }
