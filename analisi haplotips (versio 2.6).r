...
carrega.llibreria<-function(llibreries){
	...
	missing.library<-required.library[!required.library%in%.packages(all.available = TRUE)] 
	if (length(missing.library)>0) for (i in 1:length(missing.library)) install.packages(missing.library[i])
	if (sum(!paste("package:",required.library,sep="")%in%search())>0) for (i in 1:length(required.library)) library(required.library[i],character.only=TRUE)
}
...
haplo.model<-function(haplo.base,primera.model=TRUE,y=y,descriptius=FALSE){
	...
	if (!descriptius){
		...
		inter1cat<-grep("C\\(as.factor",names.beta[-inter2cat])
...
Recorda que:\n
    (1) Els missings de les variables excepte els SNP's han de ser SYSMIS\n
    ...
    (4) Els individus filtrats no han d'estar a la base de dades\n
    (5) La variable resposta ha de ser d'un dels següents dos tipus\n
        (5.1) binària (regressió logística), amb codis 1:2 o bé 0:1 ('èxit'=1)\n
        (5.2) contínua (regressió lineal)\n
    (6) Cada anàlisi es fa en UNA base de dades en SPSS:\n
...
dades.spss<-choose.files("*.sav",caption="Escull l'arxiu de dades en SPSS",multi=FALSE)

# IMPORTANT: l'arxiu de dades de SPSS ha d'incloure els casos a analitzar (el filtre s'ha de fer des de SPSS).


carrega.llibreria("foreign")
dades<-read.spss(dades.spss,use.value.labels=FALSE,to.data.frame=TRUE)
attach(dades)
...
correct.path=TRUE
path=dirname(dades.spss)
path=paste(path,"/resultats haplotips ",sub(".sav","",basename(dades.spss)),sep="")
...
while (tornar){
	...
	if (canvi.na.action){
		...
		na.action<-select.list(c("na.exclude","na.geno.keep"),title="NA action")
		if (na.action=="na.geno.keep"){
			max.missings<-menu(as.character(1:(N-1)),graphics=TRUE,"Màx missings en SNIP's")
			keep<-as.integer(apply(is.na(dades[,locus.labels]) | dades[,locus.labels]==9,1,sum))<=max.missings
		}
		if (!na.action=="na.geno.keep") keep<-!apply(is.na(dades[,locus.labels]) | (!dades[,locus.labels]==1 & !dades[,locus.labels]==2 & !dades[,locus.labels]==3),1,any)
	...
	if (!canvi.resposta & resp.true & canvi.na.action) y<-dades[keep,y.label]
	...
	if (!canvi.grup & grup.true & canvi.na.action) grup<-dades[keep,grup.label]
	...
	if (canvi.polimorfismes){ # no efecte el canvi del na.action, només els canvis en els polimorfismes
	...
	if (canvi.grup | canvi.polimorfismes | canvi.em.control | canvi.na.action){
	...
	if (resp.true & (canvi.resposta | canvi.polimorfismes | canvi.ambientals | canvi.min.freq | canvi.haplo.effect | canvi.haplo.glm.min | canvi.na.action)){
	...
	if (tornar){
		...
		sel<-select.list(c("na.action","haplo.effect","min.freq","EM control","GLM haplo freq. min"),multiple=TRUE,title="Quines opcions vols canviar?")
