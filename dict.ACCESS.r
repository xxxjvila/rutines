...
carrega.llibreria(c("gdata"))


####### crea un fitxer de diccionari de variables (millor txt) per a importar a ACCESS d'uns fitxers de sps (en el mateix format del que dona la funció dictionary.r)
...
dict.ACCESS<-function(files.sps,file.res,nom.taules=gsub(".sps$","",basename(files.sps))){
  ...
  dict.ACCESS.aux<-function(file.dict,nom.taula,append,col.names){
    ...
    if (length(begin.value.labels)){ # si té value labels
